...
export class VmService {
  ...
  async findAll(query: QueryVmDto) {
    const {
      page = 1,
      limit = 20,
      search,
    ...
    const where: Prisma.VmWhereInput = {};
    ...
    if (cluster) where.cluster = { contains: cluster, mode: 'insensitive' };
    if (kondisi) where.kondisi = { contains: kondisi, mode: 'insensitive' };
    if (statusOs) where.statusOs = { contains: statusOs, mode: 'insensitive' };
    if (kritikalitas) where.kritikalitas = { contains: kritikalitas, mode: 'insensitive' };
    if (vcenter) where.vcenter = { contains: vcenter, mode: 'insensitive' };
    if (category) where.category = { contains: category, mode: 'insensitive' };
    if (osDetail) where.osDetail = { contains: osDetail, mode: 'insensitive' };

    const normalizedDepartment = department?.trim();
    const isUnknownDepartment =
      normalizedDepartment?.toLowerCase() === 'unknown department';

    if (team || normalizedDepartment) {
      if (isUnknownDepartment) {
        // VMs are assigned to "Unknown Department" when their application has no
        // app mapping with a valid department, or when the application itself is missing.
        ...
        const unknownApplicationFilter: Prisma.VmWhereInput = {
          OR: [
            { application: null },
        ...
        const appFilter: Prisma.AppMappingWhereInput = {};
        if (team) appFilter.team = { contains: team, mode: 'insensitive' };
        ...
        const appNames = mappings.map((m) => m.aplikasi).filter(Boolean) as string[];
    ...
    // Enrich VMs with app_mapping data
    const appNames = data
      .map((v) => v.application)
      .filter(Boolean) as string[];
  ...
  async update(id: number, dto: UpdateVmDto, actor?: { id: number; username: string }) {
    await this.findOne(id);
    const vm = await this.prisma.vm.update({ where: { id }, data: dto });
    await this.audit.log({ entity: 'Vm', entityId: id, action: 'UPDATE', userId: actor?.id, username: actor?.username, payload: dto as object });
    return vm;
  }

  async remove(id: number, actor?: { id: number; username: string }) {
    await this.findOne(id);
  ...
  async getStats() {
    type GroupRow = { label: string; count: bigint };

    const [total, byKondisi, byStatusOs, byKritikalitas, byCategory, byVcenter] =
      await this.prisma.$transaction([
        this.prisma.vm.count(),
        this.prisma.$queryRaw<GroupRow[]>`SELECT kondisi AS label, COUNT(*) AS count FROM vms GROUP BY kondisi ORDER BY count DESC`,
        this.prisma.$queryRaw<GroupRow[]>`SELECT "statusOs" AS label, COUNT(*) AS count FROM vms GROUP BY "statusOs" ORDER BY count DESC`,
        this.prisma.$queryRaw<GroupRow[]>`SELECT kritikalitas AS label, COUNT(*) AS count FROM vms GROUP BY kritikalitas ORDER BY count DESC`,
        this.prisma.$queryRaw<GroupRow[]>`SELECT category AS label, COUNT(*) AS count FROM vms GROUP BY category ORDER BY count DESC`,
        this.prisma.$queryRaw<GroupRow[]>`SELECT vcenter AS label, COUNT(*) AS count FROM vms GROUP BY vcenter ORDER BY count DESC`,
      ]);

    const toList = (rows: GroupRow[]) =>
